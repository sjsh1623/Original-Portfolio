...
<body onload="buyPayOrder()">
<%@ include file="../inc/admin_header.jsp" %>
<div class="content-padder content-background">
    <div class="uk-section-xsmall uk-section-default header">
        <div class="uk-container uk-container-large">
            <h1 class="bold"><span class="ion-speedometer"></span>입찰자 입금관리</h1>
        </div>
    </div>
                        ...
                        <div>
                            <div class="uk-card uk-card-default">
                                <div class="uk-card-header bold">
                                    입금 정보 <br><br>
                                    <span class="small">* 송금 확인후 해당 탭을 클릭해 송금완료로 수정해주세요. <br>* 특정 픽셀로 내려가면 깨질수 있으니 사이드 바를 닫아주세요<br>
                                        * 현 페이지에서는 입금확인만 가능합니다.<br>* 판매자의 상품을 받았다면 바로 <mark>검수중</mark>으로 넘어갑니다.</span>
                                </div>
...
<script type="text/x-handlebars-template" id="buyList_tmpl">
    ...
    <tr class="thumbTable">
        ...
        <td>
            <button class="checkButton" onclick="updateToDone({{id}}, {{sellOrderStatus}})">입금확인</button>
        </td>
    </tr>
    {{/each}}
</script>
...
<script>
    function buyPayOrder() {
        ...
        $.post("${pageContext.request.contextPath}/admin/buyPayOrder.do", {
            key: search,
            option: options,
            nowPage: page
        }, function (json) {
            ...
            if (json.page.prevPage > 0) {
                $("#pagination").append("<li><a href=\"${pageContext.request.contextPath}/admin/manageBuyTransaction?category=" + options + "&keyword=" + search + "&page=" + json.page.prevPage + "\"><span uk-pagination-previous></span></a></li>")
            }
            ...
            for (var i = json.page.startPage; i <= json.page.endPage; i++) {
                ...
                if (i === json.page.nowPage) {
                    ...
                    $("#pagination").append("<li><a href=\"${pageContext.request.contextPath}/admin/manageBuyTransaction?category=" + options + "&keyword=" + search + "&page=" + i + "\">" + i + "</a></li>")
                }
            ...
            if (json.page.nextPage > 0) {
                $("#pagination").append("<li><a href=\"${pageContext.request.contextPath}/admin/manageBuyTransaction?category=" + options + "&keyword=" + search + "&page=" + json.page.nextPage + "\"><span uk-pagination-next></span></a></li>")
            }
        });
    }
    ...
    function updateToDone(ID, check) {
        // 만약 판매 상품을 받았다면 바로 3으로 업데이트합니다.
        console.log(check);
        if (check == 2) {
            Swal.fire({
                title: '입금확인',
                text: "판매자의 배송이 끝났으므로, 검수중으로 업데이트 됩니다.",
                icon: 'warning',
        ...
        // 판매자가 아직 배송을 완료하지 않았다면...
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.post("${pageContext.request.contextPath}/admin/orderBuyPayUpdate.do", {
                        ID: ID
                    }, function (success) {
                        if (success == 1) {
                            Swal.fire(
