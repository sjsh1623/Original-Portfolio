...
<div id="container">
    ...
    <div class="right_container">
        <p class="buy_title">
            <c:choose>
                <c:when test="${checkBuy.buyProcess eq 'N'}">
                    입찰 완료
                </c:when>
                <c:otherwise>
                    구매 완료
        ...
        <div class="option">
            <div class="option_adjustment">
                <!-- 사이즈는 파라미터로 받아옵니다.-->
                <div class="buy_left">
                    옵션 &nbsp <span id="buySize" class="option_font_size">${checkBuy.buySize}</span>
                </div>
            </div>
        </div>
        ...
        <div class="buy_shipping_price_container">
            <div class="buy_shipping_price_text">입찰 가격</div>
            <div class="buy_shipping_price" >₩
                <fmt:formatNumber value="${checkBuy.buyPrice}" pattern="#,###"/>
            </div>
        </div>
        ...
        <div class="buy_shipping_price_container">
            ...
            <div class="buy_shipping_price" id="buy_shipping">₩
                <c:choose>
                    <c:when test="${checkBuy.buyFee eq 8000}">
                        3,000
                    </c:when>
                    <c:otherwise>
                        0
        ...
        <div class="buy_shipping_price_container">
            ...
            <div class="buy_shipping_totalprice" id="buy_totalPrice">₩ <fmt:formatNumber value="${checkBuy.buyPrice + checkBuy.buyFee}"
                                                                                         pattern="#,###"/></div>
        </div>
        ...
        <!-- 입찰 마감일-->
        <c:if test="${checkBuy.buyProcess eq 'N'}">
            <div class="buy_bidDate">
                <div class="buy_shipping_price_text">입찰 마감일</div>
                <div class="buy_shipping_price">
                    <fmt:formatNumber value="${checkBuy.buyEnd}" pattern="#,###"/> 일 <span class="smallSize">(<c:out
                        value="${seven}"/>)</span>
                </div>
            </div>
        </c:if>
        ...
        <div class="buy_address_container" id="address_container">
            ...
            <div class="buy_address_info" id="userAddrInfo">${user.userAddress1}<br>${user.userAddress2}</div>
        </div>
        <hr class="buy_hr_fade">
...
<script>
    function myPage() {
        window.location.href = "<%=request.getContextPath()%>/myPage/buy"
    }
