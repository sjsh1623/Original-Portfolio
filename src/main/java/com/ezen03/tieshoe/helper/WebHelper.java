package com.ezen03.tieshoe.helper;
...
public class WebHelper {
    ...
    public void setStringSession(String key, String value) {
        // 세션의 기록 정보를 로그로 남긴다.
        log.debug(String.format("(s) <-- %s = %s", key, value));
     ...
     * @throws Exception
     */
    public String createThumbnail(String path, int width, int height, boolean crop, int file) throws Exception {
        ...
        String start = null;
        String end = null;
        File f = new File(dirPath);          // 생성될 썸네일 파일 객체 --> 업로드폴더 + 썸네일이름
        String saveFile = f.getAbsolutePath();          // 생성될 썸네일 파일 객체로부터 절대경로 추출 (리턴할 값)
        p = saveFile.lastIndexOf(":\\") + 2;

        if (file == 0) {
            thumbName = name + "_index." + ext;
            f = new File(dirPath, thumbName);          // 생성될 썸네일 파일 객체 --> 업로드폴더 + 썸네일이름
            saveFile = f.getAbsolutePath();          // 생성될 썸네일 파일 객체로부터 절대경로 추출 (리턴할 값)
            if (!(saveFile.indexOf("upload") > -1)) {
                start = saveFile.substring(0, p);
                end = saveFile.substring(p);
                saveFile = start + "index\\" + end;
            } else {
                saveFile = saveFile.replace("upload", "upload\\index");
            }
        } else {
            ...
            if (!(saveFile.indexOf("upload") > -1)) {
                ...
                saveFile = start + "thumb\\" + end;
            } else {
                saveFile = saveFile.replace("upload", "upload\\thumb");
            }
        }
